...
/**
 * Order Leak inventory: the arithmetic walk measures the dark canvas, hero,
 * warning/status surfaces, histogram labels, ciphertext and recovery tables,
 * revealed score, details, and topbar at every driven state. Its full upstream
 * paint model below is retained so translucent colors, gradients, clipping,
 * opacity groups, SVG underlays, and aria-hidden painted text stay measurable.
 */
...
export async function auditContrast(
  ...
  return page.evaluate(([rootSelector, allowAriaHidden]: [string, boolean]) => {
     ...
     *
    * A driven pass repeatedly walks both generated tables. Without the caches
    ...
    * Nothing mutates during one measurement, so cached values cannot go stale.
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // that paints nothing. `checkVisibility()` catches it. Order Leak's method
      // disclosure ships shut and the gate opens it through its real summary.
      ...
      // page's `.skip-link` — the only one — parks at `top:-5rem`
