...
/**
 * Order Leak inventory: measure boundaries for segmented scheme buttons,
 * row/population selects, query/recovery/reveal commands, topbar links styled
 * as controls, and any generated content. The complete upstream compositing
 * implementation is retained and runs after every state, hover, and focus.
 */
 ...
 *
 * IN THIS LAB the control-boundary half is live. The first capture found every
 * unselected scheme button and both selects at 2.39:1 against their panel. The
 * dedicated `--control-edge` token fixed those findings; this oracle now guards
 * that token through every mode, hover, focus, and narrow state.
 *
...
export async function auditNonText(page: Page, within = 'body *'): Promise<NonTextFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     *
    * A driven pass repeatedly walks both generated tables. Without the cache
    * it would re-read the same panel and table ancestors thousands of times.
    * Nothing mutates during one measurement, so cached values cannot go stale.
     */
    ...
    for (const el of Array.from(document.querySelectorAll(CONTROL))) {
      ...
      // Order Leak paints all four control sides today, but that remains an
      // implementation detail rather than a load-bearing oracle assumption.
      //
